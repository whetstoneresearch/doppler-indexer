...
import { DOPPLER_LENSES, STATE_VIEWS } from "@app/config/const";
...
export const getV4PoolData = async ({
  ...
  const stateView = STATE_VIEWS[context.chain!.name];
...
export const getLatestSqrtPrice = async ({
  ...
  const { client, chain } = context;
  const lensQuoter = DOPPLER_LENSES[chain!.name];
...
export const getReservesV4Zora = async ({
  ...
  const stateView = STATE_VIEWS[chain!.name];
