...
ponder.on("ZoraFactory:CoinCreatedV4", async ({ event, context }) => {
  ...
  const isQuoteZora = currency != zeroAddress && currency.toLowerCase() === zoraAddresses.zoraToken.toLowerCase();
  const isQuoteEth = currency === zeroAddress || currency.toLowerCase() === SHARED_ADDRESSES.weth.toLowerCase();
...
ponder.on("ZoraContentCoinHook:Swapped", async ({ event, context }) => {
...
ponder.on("ZoraCreatorCoinHook:Swapped", async ({ event, context }) => {
  const { poolKeyHash, swapSender, amount0, amount1, sqrtPriceX96, isCoinBuy } = event.args;
  const timestamp = event.block.timestamp;
...
ponder.on("ZoraCreatorCoin:LiquidityMigrated", async ({ event, context }) => {
  ...
  const isQuoteZora = fromPoolEntity.quoteToken.toLowerCase() === zoraAddresses.zoraToken.toLowerCase();
  const isQuoteEth = fromPoolEntity.quoteToken.toLowerCase() === SHARED_ADDRESSES.weth.toLowerCase();
...
ponder.on("ZoraCreatorCoin:CoinTransfer", async ({ event, context }) => {
...
ponder.on("ZoraContentCoin:CoinTransfer", async ({ event, context }) => {
  const { address } = event.log;
  const { timestamp } = event.block;
  const { sender, recipient, senderBalance, recipientBalance } = event.args;
